    ...
    $maxLines = 3;
        ...
        if ($result) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<img src="' . $row['manga_pick'] . '" alt="' . $row['manga_name'] . '">';
                echo '<div class="anime-info">';
                echo '<h3>' . $row['manga_name'] . '</h3>';
                echo '<button class="read-more" data-name="' . htmlspecialchars($row['manga_name']) . '" data-description="' . htmlspecialchars($row['manga__description']) . '">Узнать</button>';
                echo '</div>';
                echo '</div>';
            }
        } else {
